...
class FrontRunners(object):
    ...
    def tally(self):
        ...
        elif self.kind == 'condorcet':
            tally = self.get_tally_condorcet()
            
        elif self.kind == 'regret':
            tally = self.get_tally_regret()     
        return tally
            
    
    @lazy_property
    def frontrunners(self):
        """array : Election frontrunners with the honest winner as first entry."""
        return self.get_frontrunners()        
        
        
        
    def get_frontrunners(self):
        if self.kind == 'eliminate':
            return self.get_frontrunners_elimination()
        else:
            return self.get_frontunners_tally()
        
        
    def get_frontunners_tally(self):
        ...
        winners, ties = votemethods.tools.winner_check(tally,
                                                       numwin=numwinners)
        frontrunners = list(winners) +  ties
        ...
        flist2 = additional_tally_frontrunners(tally, tol=self.tol)
    ...
    def get_frontrunners_elimination(self):
        
        ballots = self.election.ballots
        er = self.election.result.runner
        numwinners = self.numwinners
        frunners = []
    ...
    def get_tally_method(self):
        ...
        try:
            tally = self.election.result.runner.output['tally']
            return tally
    ...
    def get_tally_condorcet(self):
        ...
        output = self.election.result.output
