...
from votesim import metrics, randomstate
...
class ElectionRun(object):
    ...
    def _run_behavior(self):
        ...
        ratings = vcalcs.voter_scores_by_tolerance(self.voters, 
                                                    self.candidates,
                                                    tol=self.tol,
                                                    error_std=self.error,
                                                    cnum = self.cnum,
                                                    strategy=strategy)
        ranks = vcalcs.score2rank(ratings)
        scores = np.round(ratings * scoremax)
        self.ranks = ranks
        self.scores = scores
        self.ratings = ratings
        self.votes = vcalcs.getplurality(ranks=ranks)
        return
