...
#    width1, width2  = rs.uniform(.25, 1, 2)
    ...
    tolerance = rs.uniform(1.0, 7.0, 1)[0]
    voter_std = np.std(voters)
    voter_mean = np.mean(voters)    
    candidates = rs.uniform(-2*voter_std, 2*voter_std,(numcandidates, 2)) + voter_mean
...
#    candidates1 = candidates[:, None]
    ...
    e = sim.ElectionRun(voters, candidates,
    ...
    if build_plots and trial_no < 10:
        title = 'Trial-2d- #%d' % trial_no
        sim.plot2d(e, results, title)
